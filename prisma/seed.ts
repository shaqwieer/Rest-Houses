...
async function main() {
  ...
  for (const [index, item] of LISTINGS.entries()) {
    ...
    const data = {
      ...
      // Sharjah works a four-day week, so a rest house there is full on Sunday
      // night too and prices three weekend nights rather than two. Seeded from
      // the emirate so the sample catalogue demonstrates both settings — with
      // every listing on "short" the difference would never show up anywhere.
